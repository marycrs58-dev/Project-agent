...
function Badge({label}){
  const m={Completado:{bg:COLORS.greenLight,c:COLORS.green},"En progreso":{bg:COLORS.tealLight,c:COLORS.teal},Pendiente:{bg:COLORS.amberLight,c:COLORS.amber},Bloqueado:{bg:COLORS.redLight,c:COLORS.red},Alta:{bg:COLORS.redLight,c:COLORS.red},Media:{bg:COLORS.amberLight,c:COLORS.amber},Baja:{bg:COLORS.greenLight,c:COLORS.green},admin:{bg:"#EDE9FE",c:"#5B21B6"},viewer:{bg:COLORS.grayLight,c:COLORS.gray}};
  const s=m[label]||{bg:COLORS.grayLight,c:COLORS.gray};
...
function Btn({children,v="primary",...p}){const s={primary:{background:COLORS.teal,color:"#fff",border:"none"},secondary:{background:COLORS.surface,color:COLORS.gray,border:`1px solid ${COLORS.border}`},danger:{background:COLORS.redLight,color:COLORS.red,border:"none"},ghost:{background:"transparent",color:COLORS.teal,border:`1px solid ${COLORS.teal}`},purple:{background:"#EDE9FE",color:"#5B21B6",border:"none"}};return <button {...p} style={{padding:"7px 15px",borderRadius:8,cursor:"pointer",fontSize:13,fontWeight:500,whiteSpace:"nowrap",...(s[v]||s.primary),...(p.style||{})}}>{children}</button>;}
function Field({label,children}){return(<div><label style={{fontSize:11,color:COLORS.gray,display:"block",marginBottom:4,textTransform:"uppercase",letterSpacing:"0.04em"}}>{label}</label>{children}</div>);}
function Inp({...p}){return <input {...p} style={{width:"100%",padding:"8px 11px",borderRadius:7,border:`1px solid ${COLORS.border}`,fontSize:13,boxSizing:"border-box",outline:"none",background:COLORS.surface,fontFamily:"inherit",...(p.style||{})}}/>;}
function Sel({children,...p}){return <select {...p} style={{width:"100%",padding:"8px 11px",borderRadius:7,border:`1px solid ${COLORS.border}`,fontSize:13,boxSizing:"border-box",outline:"none",background:COLORS.surface,fontFamily:"inherit",...(p.style||{})}}>{children}</select>;}
...
function Spinner(){return <div style={{width:32,height:32,border:`3px solid ${COLORS.border}`,borderTopColor:COLORS.teal,borderRadius:"50%",animation:"spin 0.8s linear infinite"}}><style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style></div>;}
...
function LoginScreen({onLogin}){
  ...
  return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:COLORS.bg,padding:16}}>
      <div style={{background:COLORS.surface,borderRadius:20,padding:36,width:"100%",maxWidth:400,boxShadow:"0 8px 40px rgba(0,0,0,0.1)"}}>
        {/* Logo */}
        <div style={{textAlign:"center",marginBottom:28}}>
          <div style={{width:52,height:52,borderRadius:14,background:COLORS.teal,display:"flex",alignItems:"center",justifyContent:"center",fontSize:24,margin:"0 auto 12px"}}>💊</div>
          <div style={{fontSize:20,fontWeight:800,color:COLORS.text,letterSpacing:-0.5}}>Gestión de Proyectos</div>
          <div style={{fontSize:13,color:COLORS.gray,marginTop:2}}>OTC · Cosméticos · Manufactura</div>
        </div>

        {msg&&<div style={{background:COLORS.greenLight,color:COLORS.green,borderRadius:8,padding:"10px 14px",fontSize:13,marginBottom:16,textAlign:"center"}}>{msg}</div>}
        {error&&<div style={{background:COLORS.redLight,color:COLORS.red,borderRadius:8,padding:"10px 14px",fontSize:13,marginBottom:16}}>{error}</div>}
        ...
        {mode==="login"&&(
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            <button onClick={handleLogin} disabled={loading}
              style={{padding:"11px 0",borderRadius:9,border:"none",background:COLORS.teal,color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",marginTop:4,display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
              {loading?<Spinner/>:"Iniciar sesión"}
            </button>
            <div style={{textAlign:"center",fontSize:13,color:COLORS.gray,display:"flex",gap:12,justifyContent:"center",marginTop:4}}>
              <button onClick={()=>{setMode("register");setError("");}} style={{background:"none",border:"none",color:COLORS.teal,cursor:"pointer",fontSize:13}}>Crear cuenta</button>
              <span>·</span>
              <button onClick={()=>{setMode("forgot");setError("");}} style={{background:"none",border:"none",color:COLORS.gray,cursor:"pointer",fontSize:13}}>¿Olvidaste tu contraseña?</button>
            </div>
          </div>
        )}
        ...
        {mode==="register"&&(
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            ...
            <button onClick={()=>{setMode("login");setError("");}} style={{background:"none",border:"none",color:COLORS.gray,cursor:"pointer",fontSize:13,textAlign:"center"}}>← Volver al login</button>
          </div>
        )}

        {mode==="forgot"&&(
          <div style={{display:"flex",flexDirection:"column",gap:12}}>
            <div style={{fontSize:13,color:COLORS.gray,marginBottom:4}}>Ingresa tu email y te enviaremos un link para restablecer tu contraseña.</div>
            <Field label="Email"><Inp type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="tu@email.com"/></Field>
            <button onClick={handleForgot} disabled={loading}
              style={{padding:"11px 0",borderRadius:9,border:"none",background:COLORS.teal,color:"#fff",fontSize:14,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:8}}>
...
function UsuariosModal({onClose}){
  ...
  return(<Overlay><div style={{background:COLORS.surface,borderRadius:16,padding:24,width:"92%",maxWidth:540,maxHeight:"88vh",overflowY:"auto"}}>
    <div style={{fontSize:16,fontWeight:700,color:COLORS.text,marginBottom:4}}>Gestión de usuarios</div>
    <div style={{fontSize:13,color:COLORS.gray,marginBottom:18}}>Los <b>admin</b> pueden crear/editar proyectos. Los <b>viewer</b> solo pueden consultar.</div>

    {/* Lista usuarios */}
    {loading?<div style={{textAlign:"center",padding:20}}><Spinner/></div>:
      <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:20}}>
        {usuarios.map(u=>(
          <div key={u.id} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",background:COLORS.grayLight,borderRadius:9}}>
            <Avatar nombre={u.nombre||u.email} size={32}/>
            <div style={{flex:1}}>
              <div style={{fontSize:13,fontWeight:600,color:COLORS.text}}>{u.nombre||"Sin nombre"}</div>
              <div style={{fontSize:11,color:COLORS.gray}}>{u.email}</div>
            </div>
    ...
    {/* Crear usuario */}
    <div style={{background:COLORS.tealLight,borderRadius:10,padding:16}}>
      <div style={{fontSize:13,fontWeight:600,color:COLORS.teal,marginBottom:12}}>Crear nuevo usuario</div>
      {msg&&<div style={{background:COLORS.greenLight,color:COLORS.green,borderRadius:7,padding:"8px 12px",fontSize:12,marginBottom:10}}>{msg}</div>}
      {err&&<div style={{background:COLORS.redLight,color:COLORS.red,borderRadius:7,padding:"8px 12px",fontSize:12,marginBottom:10}}>{err}</div>}
...
function ResponsablesModal({equipo,onSave,onClose}){
  ...
  return(<Overlay><div style={{background:COLORS.surface,borderRadius:16,padding:24,width:"92%",maxWidth:480,maxHeight:"85vh",overflowY:"auto"}}>
    <div style={{fontSize:16,fontWeight:700,color:COLORS.text,marginBottom:18}}>Gestionar responsables</div>
    <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
      {lista.map(e=><div key={e.id} style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",background:COLORS.grayLight,borderRadius:8}}>
        <Avatar nombre={e.nombre} size={28}/><div style={{flex:1}}><div style={{fontSize:13,fontWeight:600}}>{e.nombre}</div><div style={{fontSize:11,color:COLORS.gray}}>{e.rol}</div></div>
        <button onClick={()=>del(e.id)} style={{background:"none",border:"none",cursor:"pointer",color:COLORS.red,fontSize:16}}>✕</button>
      </div>)}
    </div>
    <div style={{background:COLORS.tealLight,borderRadius:10,padding:14,marginBottom:16}}>
      <div style={{fontSize:12,fontWeight:600,color:COLORS.teal,marginBottom:10}}>Agregar responsable</div>
...
function EtapaModal({etapa,idx,proyecto,onSave,onClose}){
  ...
  return(<Overlay><div style={{background:COLORS.surface,borderRadius:16,padding:26,width:"92%",maxWidth:520,maxHeight:"90vh",overflowY:"auto"}}>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:20}}>
      <span style={{fontSize:22}}>{etapa.icon}</span>
      <div><div style={{fontSize:16,fontWeight:700,color:COLORS.text}}>{etapa.label}</div><div style={{fontSize:12,color:COLORS.gray}}>Paso {idx+1}</div></div>
    </div>
    <div style={{display:"flex",flexDirection:"column",gap:14}}>
      ...
      <Field label="Notas"><textarea value={form.notas} onChange={e=>set("notas",e.target.value)} rows={3} style={{width:"100%",padding:"8px 11px",borderRadius:7,border:`1px solid ${COLORS.border}`,fontSize:13,resize:"vertical",boxSizing:"border-box",outline:"none",fontFamily:"inherit"}}/></Field>
      <Field label="Entregables">
        <div style={{display:"flex",flexDirection:"column",gap:7,marginTop:4}}>
          {form.entregables.map((ent,i)=>(
            <div key={ent.id} style={{display:"flex",alignItems:"center",gap:8,padding:"7px 10px",background:ent.done?COLORS.greenLight:COLORS.grayLight,borderRadius:8,border:`1px solid ${ent.done?COLORS.green+"44":COLORS.border}`}}>
              <div onClick={()=>toggleEnt(i)} style={{width:18,height:18,borderRadius:4,border:`2px solid ${ent.done?COLORS.green:COLORS.border}`,background:ent.done?COLORS.green:"#fff",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,cursor:"pointer"}}>
                {ent.done&&<span style={{color:"#fff",fontSize:11}}>✓</span>}
              </div>
              <span onClick={()=>toggleEnt(i)} style={{fontSize:13,flex:1,cursor:"pointer",color:ent.done?COLORS.green:COLORS.text,textDecoration:ent.done?"line-through":"none"}}>{ent.desc}</span>
              <button onClick={()=>delEnt(i)} style={{background:"none",border:"none",cursor:"pointer",color:COLORS.red,fontSize:14,padding:"0 2px"}}>✕</button>
            </div>
          ))}
...
function EtapasConfigModal({etapas,onSave,onClose}){
  ...
  return(<Overlay><div style={{background:COLORS.surface,borderRadius:16,padding:24,width:"92%",maxWidth:540,maxHeight:"90vh",overflowY:"auto"}}>
    <div style={{fontSize:16,fontWeight:700,color:COLORS.text,marginBottom:4}}>Configurar etapas del proceso</div>
    <div style={{fontSize:13,color:COLORS.gray,marginBottom:16}}>Reordena, agrega o elimina etapas.</div>
    <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:16}}>
      {lista.map((e,i)=>(
        <div key={e.id} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",background:COLORS.grayLight,borderRadius:8}}>
          <span style={{fontSize:16}}>{e.icon}</span>
          <span style={{flex:1,fontSize:13,fontWeight:500}}>{e.label}</span>
          <button onClick={()=>i>0&&move(i,-1)} style={{background:"none",border:"none",cursor:"pointer",color:COLORS.gray,fontSize:14,opacity:i===0?0.3:1}}>↑</button>
          <button onClick={()=>i<lista.length-1&&move(i,1)} style={{background:"none",border:"none",cursor:"pointer",color:COLORS.gray,fontSize:14,opacity:i===lista.length-1?0.3:1}}>↓</button>
          <button onClick={()=>del(e.id)} style={{background:"none",border:"none",cursor:"pointer",color:COLORS.red,fontSize:14}}>✕</button>
        </div>
      ))}
    </div>
    <div style={{background:COLORS.tealLight,borderRadius:10,padding:14,marginBottom:16}}>
      <div style={{fontSize:12,fontWeight:600,color:COLORS.teal,marginBottom:10}}>Agregar etapa</div>
    ...
    <div style={{overflowX:"auto",paddingBottom:8}}>
      <div style={{display:"flex",alignItems:"flex-start",gap:0,minWidth:"max-content"}}>
        {proyecto.etapas.map((etapa,i)=>{
          ...
          const bg=done?COLORS.teal:isActual?proyecto.color:"#E8E4DC";
          return(
            <div key={etapa.id} style={{display:"flex",alignItems:"center"}}>
              <div onClick={()=>onEtapaClick(etapa,i)} style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,cursor:"pointer",padding:"0 4px"}}>
                ...
                <span style={{fontSize:9,color:done?COLORS.teal:isActual?proyecto.color:COLORS.gray,fontWeight:isActual?700:500,maxWidth:58,textAlign:"center",lineHeight:1.2}}>{etapa.label}</span>
                {(etapa.fechaInicio||etapa.fechaFin)&&(
                  <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:1,marginTop:2}}>
                    {etapa.fechaInicio&&<span style={{fontSize:8,color:COLORS.teal,fontWeight:600,background:COLORS.tealLight,borderRadius:3,padding:"1px 4px"}}>{fmtDate(etapa.fechaInicio)}</span>}
                    {etapa.fechaFin&&<span style={{fontSize:8,color:COLORS.amber,fontWeight:600,background:COLORS.amberLight,borderRadius:3,padding:"1px 4px"}}>{fmtDate(etapa.fechaFin)}</span>}
                  </div>
                )}
              </div>
              {i<proyecto.etapas.length-1&&<div style={{width:18,height:2,background:done?COLORS.teal:"#E8E4DC",marginBottom:etapa.fechaFin?28:18,flexShrink:0}}/>}
            </div>
          );
        })}
      </div>
      <div style={{display:"flex",gap:12,marginTop:6,fontSize:10,color:COLORS.gray}}>
        <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:2,background:COLORS.teal,display:"inline-block"}}/>Inicio</span>
        <span style={{display:"flex",alignItems:"center",gap:4}}><span style={{width:8,height:8,borderRadius:2,background:COLORS.amber,display:"inline-block"}}/>Compromiso</span>
      </div>
    </div>
  );
}
...
function GanttTab({proyecto}){
  ...
  const barColor={Completado:COLORS.teal,"En progreso":proyecto.color,Pendiente:"#D1D5DB",Bloqueado:COLORS.red};
  const exportCSV=()=>{
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <span style={{fontSize:13,fontWeight:600,color:COLORS.text}}>Gantt — {proyecto.nombre}</span>
      ...
      <div style={{overflowX:"auto"}}>
        <div style={{display:"flex",marginLeft:180,marginBottom:4}}>
          {months.map((m,i)=>{const mStart=new Date(m.year,m.month,1);const mEnd=new Date(m.year,m.month+1,0);const pctStart=Math.max(0,Math.ceil((mStart-start)/86400000)/totalDays*100);const pctEnd=Math.min(100,Math.ceil((mEnd-start)/86400000)/totalDays*100);const width=pctEnd-pctStart;return <div key={i} style={{flex:`0 0 ${width}%`,fontSize:10,color:COLORS.gray,borderLeft:`1px dashed ${COLORS.border}`,paddingLeft:4,overflow:"hidden",whiteSpace:"nowrap"}}>{m.label}</div>;})}
        </div>
        <div style={{position:"relative"}}>
          <div style={{position:"absolute",left:`calc(180px + ${todayPct}%)`,top:0,bottom:0,width:2,background:COLORS.red,zIndex:3,opacity:0.6}}/>
          {proyecto.etapas.map((etapa)=>{
            ...
            return(
              <div key={etapa.id} style={{display:"flex",alignItems:"center",marginBottom:7}}>
                <div style={{width:180,flexShrink:0,fontSize:12,color:COLORS.text,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",paddingRight:8,display:"flex",alignItems:"center",gap:6}}><span>{etapa.icon}</span><span>{etapa.label}</span></div>
      ...
      <div style={{display:"flex",gap:14,marginTop:10,flexWrap:"wrap"}}>
        {[["Completado",COLORS.teal],["En progreso",proyecto.color],["Pendiente","#D1D5DB"],["Hoy",COLORS.red]].map(([l,co])=>(<div key={l} style={{display:"flex",alignItems:"center",gap:5,fontSize:11,color:COLORS.gray}}><div style={{width:10,height:10,borderRadius:2,background:co}}/>{l}</div>))}
      </div>
      <div style={{marginTop:14,padding:12,background:COLORS.grayLight,borderRadius:10,fontSize:12,color:COLORS.gray}}>💡 Las barras aparecen cuando asignas fechas inicio y fin en cada etapa. Exporta a CSV para abrir en Excel.</div>
    </div>
  );
}
...
function CalendarioTab({proyecto}){
  ...
  proyecto.tareas.forEach(t=>addEvt(t.fechaLimite,{label:t.nombre,color:COLORS.amber}));
  proyecto.etapas.forEach(e=>{addEvt(e.fechaInicio,{label:`▶ ${e.label}`,color:COLORS.teal});addEvt(e.fechaFin,{label:`⏱ ${e.label}`,color:proyecto.color});});
  ...
  return(
    <div>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
        ...
        <span style={{fontWeight:700,fontSize:15,color:COLORS.text,flex:1,textAlign:"center"}}>{meses[month]} {year}</span>
      ...
      <div style={{display:"flex",gap:10,marginBottom:10,flexWrap:"wrap",fontSize:11}}>
        {[["Tarea",COLORS.amber],["Inicio etapa",COLORS.teal],["Fin etapa",proyecto.color]].map(([l,c])=>(<span key={l} style={{display:"flex",alignItems:"center",gap:4,color:COLORS.gray}}><span style={{width:8,height:8,borderRadius:"50%",background:c,display:"inline-block"}}/>{l}</span>))}
      </div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2}}>
        {["Do","Lu","Ma","Mi","Ju","Vi","Sa"].map(d=><div key={d} style={{textAlign:"center",fontSize:11,fontWeight:600,color:COLORS.gray,padding:"4px 0"}}>{d}</div>)}
        {cells.map((day,i)=>{
          const evts=day?(eventos[day]||[]):[];const isToday=day===now.getDate()&&month===now.getMonth()&&year===now.getFullYear();
          return(<div key={i} style={{minHeight:64,borderRadius:7,padding:"3px 5px",background:day?(isToday?"#EFF6FF":"#FAFAFA"):"transparent",border:day?`1px solid ${isToday?COLORS.teal:COLORS.border}`:"none"}}>
            {day&&<div style={{fontSize:11,fontWeight:isToday?700:400,color:isToday?COLORS.teal:COLORS.text,marginBottom:2}}>{day}</div>}
            ...
            {evts.length>3&&<div style={{fontSize:9,color:COLORS.gray}}>+{evts.length-3}</div>}
          </div>);
        })}
      </div>
    </div>
...
function TareaModal({tarea,equipo,onSave,onClose}){
  ...
  return(<Overlay><div style={{background:COLORS.surface,borderRadius:16,padding:24,width:"92%",maxWidth:440}}>
    <div style={{fontSize:15,fontWeight:700,color:COLORS.text,marginBottom:18}}>{tarea?"Editar tarea":"Nueva tarea"}</div>
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
...
function MinutaModal({proyecto,onSave,onClose}){
  ...
  return(<Overlay><div style={{background:COLORS.surface,borderRadius:16,padding:24,width:"92%",maxWidth:520,maxHeight:"92vh",overflowY:"auto"}}>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
      ...
      <div><div style={{fontSize:15,fontWeight:700,color:COLORS.text}}>Minuta de reunión</div><div style={{fontSize:12,color:COLORS.gray}}>{proyecto.nombre}</div></div>
    </div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      ...
      <Field label="Asistentes">
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:4}}>
          {nombres.map(n=>(<div key={n} onClick={()=>toggleAs(n)} style={{display:"flex",alignItems:"center",gap:6,padding:"4px 10px",borderRadius:20,background:form.asistentes.includes(n)?COLORS.tealLight:COLORS.grayLight,border:`1px solid ${form.asistentes.includes(n)?COLORS.teal+"66":COLORS.border}`,cursor:"pointer",fontSize:12}}><Avatar nombre={n} size={18}/><span style={{color:form.asistentes.includes(n)?COLORS.teal:COLORS.gray}}>{n.split(" ")[0]}</span></div>))}
        </div>
      </Field>
      <Field label="Acuerdos (uno por línea)"><textarea value={form.acuerdos} onChange={e=>set("acuerdos",e.target.value)} rows={4} style={{width:"100%",padding:"8px 11px",borderRadius:7,border:`1px solid ${COLORS.border}`,fontSize:13,resize:"vertical",boxSizing:"border-box",outline:"none",fontFamily:"inherit"}}/></Field>
      <Field label="Próximos pasos"><textarea value={form.proximos} onChange={e=>set("proximos",e.target.value)} rows={3} style={{width:"100%",padding:"8px 11px",borderRadius:7,border:`1px solid ${COLORS.border}`,fontSize:13,resize:"vertical",boxSizing:"border-box",outline:"none",fontFamily:"inherit"}}/></Field>
    </div>
    {fase==="generando"&&<div style={{marginTop:14,padding:12,background:COLORS.tealLight,borderRadius:10,textAlign:"center",color:COLORS.teal,fontSize:13}}>⏳ Generando minuta…</div>}
    {fase==="done"&&<div style={{marginTop:14,padding:12,background:COLORS.greenLight,borderRadius:10,color:COLORS.green,fontSize:13}}>✅ Minuta generada. Revisa y guarda.</div>}
...
function TeamsModal({tareas,onClose}){
  ...
  return(<Overlay><div style={{background:COLORS.surface,borderRadius:16,padding:24,width:"92%",maxWidth:420}}>
    <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
      ...
      <div><div style={{fontSize:15,fontWeight:700}}>Microsoft Teams</div><div style={{fontSize:12,color:COLORS.gray}}>Recordatorio tareas vencidas</div></div>
    </div>
    {!sent?(<>
      {tareas.length===0?<div style={{color:COLORS.green,marginBottom:16}}>✅ Sin tareas vencidas.</div>:tareas.map(t=>(<div key={t.id} style={{display:"flex",justifyContent:"space-between",padding:"8px 12px",background:COLORS.amberLight,borderRadius:8,marginBottom:7,fontSize:13}}><div><div style={{fontWeight:600,color:COLORS.text}}>{t.nombre}</div><div style={{fontSize:11,color:COLORS.gray}}>Vence: {t.fechaLimite}</div></div><div style={{display:"flex",alignItems:"center",gap:6}}><Avatar nombre={t.responsable||"?"} size={24}/><span style={{fontSize:12,color:COLORS.gray}}>{(t.responsable||"").split(" ")[0]}</span></div></div>))}
      ...
      <div style={{textAlign:"center",padding:"12px 0"}}>
        ...
        <div style={{fontSize:15,fontWeight:700,color:COLORS.green,marginBottom:4}}>¡Recordatorios enviados!</div>
        <div style={{fontSize:13,color:COLORS.gray,marginBottom:18}}>Los responsables recibirán un mensaje en Microsoft Teams</div>
...
function ProyectoModal({proyecto,equipoGlobal,onSave,onClose}){
  ...
  return(<Overlay><div style={{background:COLORS.surface,borderRadius:16,padding:26,width:"92%",maxWidth:520,maxHeight:"92vh",overflowY:"auto"}}>
    <div style={{fontSize:16,fontWeight:700,color:COLORS.text,marginBottom:20}}>{proyecto?"Editar proyecto":"Nuevo proyecto"}</div>
    <div style={{display:"flex",flexDirection:"column",gap:13}}>
      ...
      <Field label="Descripción"><textarea value={form.descripcion} onChange={e=>set("descripcion",e.target.value)} rows={2} style={{width:"100%",padding:"8px 11px",borderRadius:7,border:`1px solid ${COLORS.border}`,fontSize:13,resize:"none",boxSizing:"border-box",outline:"none",fontFamily:"inherit"}}/></Field>
      <Field label="Equipo asignado">
        <div style={{display:"flex",flexDirection:"column",gap:6,marginTop:4}}>
          {equipoGlobal.map((e,i)=>(<label key={e.id} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 10px",borderRadius:7,background:equipoCheck[i]?COLORS.tealLight:COLORS.grayLight,cursor:"pointer",border:`1px solid ${equipoCheck[i]?COLORS.teal+"55":COLORS.border}`}}><input type="checkbox" checked={equipoCheck[i]} onChange={()=>setEquipoCheck(prev=>prev.map((v,j)=>j===i?!v:v))} style={{accentColor:COLORS.teal}}/><Avatar nombre={e.nombre} size={24}/><span style={{fontSize:13,flex:1}}>{e.nombre}</span><span style={{fontSize:11,color:COLORS.gray,background:COLORS.surface,padding:"1px 8px",borderRadius:10}}>{e.rol}</span></label>))}
        </div>
      </Field>
      <Field label="Color del proyecto">
        <div style={{display:"flex",gap:8,marginTop:4}}>{colores.map(col=><div key={col} onClick={()=>set("color",col)} style={{width:26,height:26,borderRadius:"50%",background:col,cursor:"pointer",outline:form.color===col?`3px solid ${COLORS.text}`:"3px solid transparent",outlineOffset:2}}/>)}</div>
      </Field>
    </div>
    <div style={{display:"flex",gap:10,marginTop:20}}>
      <Btn v="secondary" style={{flex:1}} onClick={onClose}>Cancelar</Btn>
...
function TarjetaProyecto({proyecto,onClick}){
  ...
  return(
    <div onClick={onClick} style={{background:COLORS.surface,border:`1px solid ${COLORS.border}`,borderRadius:14,padding:18,cursor:"pointer",borderLeft:`5px solid ${proyecto.color}`,transition:"box-shadow 0.2s,transform 0.15s"}}
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:10}}>
        <div style={{flex:1}}>
          <div style={{fontSize:16,fontWeight:700,color:COLORS.text,marginBottom:3}}>{proyecto.nombre}</div>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}><Pill label={proyecto.categoria} color={proyecto.color}/><Pill label={proyecto.forma} color={COLORS.gray}/><span style={{fontSize:12,color:COLORS.gray}}>🏭 {proyecto.maquilador} · {proyecto.pais}</span></div>
        </div>
        {vencidas>0&&<span style={{background:COLORS.redLight,color:COLORS.red,borderRadius:20,padding:"2px 9px",fontSize:11,fontWeight:600}}>⚠️ {vencidas} vencida(s)</span>}
      </div>
      <Bar value={pct} color={proyecto.color}/>
      <div style={{display:"flex",justifyContent:"space-between",marginTop:6,fontSize:12}}>
        <span style={{color:COLORS.gray}}>{pct}% · <b style={{color:proyecto.color}}>{etapaActual?.label}</b></span>
        <span style={{color:dias<30?COLORS.red:COLORS.gray,fontWeight:dias<30?600:400}}>{dias>0?`${dias} días para lanzamiento`:"⚠️ Fecha vencida"}</span>
      </div>
      <div style={{display:"flex",gap:2,marginTop:10,overflowX:"auto"}}>
        {proyecto.etapas.map((e,i)=><div key={i} title={e.label} style={{flex:1,minWidth:6,height:6,borderRadius:3,background:e.status==="Completado"?COLORS.teal:e.status==="En progreso"?proyecto.color:"#E8E4DC"}}/>)}
      </div>
      <div style={{display:"flex",alignItems:"center",gap:6,marginTop:10}}>
        <Avatar nombre={proyecto.responsable} size={20}/><span style={{fontSize:11,color:COLORS.gray}}>{proyecto.responsable}</span>
        <span style={{marginLeft:"auto",fontSize:11,color:COLORS.gray}}>👥 {proyecto.equipo.length} · 📋 {proyecto.tareas.length} tareas</span>
      </div>
    </div>
  );
}
  ...
  return(
    <div style={{fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",maxWidth:880,margin:"0 auto",padding:"0 16px 40px"}}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:18,paddingTop:4}}>
        <button onClick={onBack} style={{background:"none",border:"none",cursor:"pointer",color:COLORS.teal,fontSize:22,padding:0,lineHeight:1}}>‹</button>
        <div style={{width:14,height:14,borderRadius:"50%",background:proyecto.color,flexShrink:0}}/>
        <div style={{flex:1}}><div style={{fontSize:20,fontWeight:800,color:COLORS.text,letterSpacing:-0.5}}>{proyecto.nombre}</div><div style={{fontSize:13,color:COLORS.gray}}>{proyecto.cliente} · {proyecto.maquilador}, {proyecto.pais}</div></div>
        <Pill label={proyecto.categoria} color={proyecto.color}/>
        {esAdmin&&<button onClick={()=>setEditProyecto(true)} style={{background:"none",border:"none",cursor:"pointer",color:COLORS.gray,fontSize:18,padding:"2px 6px"}} title="Editar">⚙️</button>}
      </div>

      <div style={{background:COLORS.surface,border:`1px solid ${COLORS.border}`,borderRadius:14,padding:"14px 16px",marginBottom:14}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
          <span style={{fontSize:13,fontWeight:700,color:COLORS.text}}>Proceso de desarrollo — {progreso(proyecto)}% completado</span>
          <div style={{display:"flex",gap:8}}>
            <span style={{fontSize:12,color:COLORS.gray}}>🚀 {proyecto.fechaLanzamiento}</span>
      ...
      <div style={{display:"flex",gap:0,borderBottom:`2px solid ${COLORS.border}`,marginBottom:18,overflowX:"auto"}}>
        {TABS.map(t=>(<button key={t} onClick={()=>setTab(t)} style={{padding:"9px 14px",border:"none",background:"none",cursor:"pointer",fontSize:13,color:tab===t?COLORS.teal:COLORS.gray,fontWeight:tab===t?700:500,borderBottom:`2px solid ${tab===t?COLORS.teal:"transparent"}`,marginBottom:-2,whiteSpace:"nowrap"}}>{t}</button>))}
      </div>

      {tab==="Proceso"&&(
        <div>
          <div style={{fontSize:12,color:COLORS.gray,marginBottom:12}}>{esAdmin?"Haz clic en una etapa para actualizar estado, fechas, responsable y entregables.":"Vista de solo lectura."}</div>
          <div style={{display:"flex",flexDirection:"column",gap:7}}>
            {proyecto.etapas.map((etapa,i)=>{
              ...
              return(<div key={etapa.id} onClick={esAdmin?()=>setModalEtapa({etapa,idx:i}):undefined}
                style={{display:"flex",alignItems:"center",gap:12,padding:"10px 14px",background:done?COLORS.greenLight:isActual?COLORS.tealLight:COLORS.surface,border:`1px solid ${done?COLORS.green+"44":isActual?COLORS.teal+"44":COLORS.border}`,borderRadius:10,cursor:esAdmin?"pointer":"default"}}>
                <span style={{fontSize:18}}>{etapa.icon}</span>
                <div style={{flex:1}}>
                  <div style={{fontSize:13,fontWeight:600,color:done?COLORS.green:isActual?COLORS.teal:COLORS.text}}>{etapa.label}</div>
                  <div style={{fontSize:11,color:COLORS.gray,display:"flex",gap:10,flexWrap:"wrap"}}><span>{entsDone}/{etapa.entregables.length} entregables</span>{etapa.fechaInicio&&<span>▶ {etapa.fechaInicio}</span>}{etapa.fechaFin&&<span>⏱ {etapa.fechaFin}</span>}{etapa.responsable&&<span>👤 {etapa.responsable}</span>}</div>
                </div>
        ...
        <div>
          <div style={{display:"flex",gap:8,marginBottom:12,flexWrap:"wrap",alignItems:"center"}}>
            <div style={{display:"flex",gap:4,flex:1,flexWrap:"wrap"}}>
              {["Todas",...ESTADOS_TAREA].map(s=>(<button key={s} onClick={()=>setFilterTarea(s)} style={{padding:"5px 11px",borderRadius:7,border:`1px solid ${filterTarea===s?COLORS.teal:COLORS.border}`,background:filterTarea===s?COLORS.tealLight:COLORS.surface,color:filterTarea===s?COLORS.teal:COLORS.gray,fontSize:12,cursor:"pointer",fontWeight:filterTarea===s?700:400}}>{s}</button>))}
            </div>
          ...
          {vencidas.length>0&&<div style={{background:COLORS.amberLight,border:`1px solid ${COLORS.amber}44`,borderRadius:10,padding:"9px 14px",marginBottom:12,fontSize:13,color:COLORS.amber}}>⚠️ {vencidas.length} tarea(s) vencida(s) requieren atención</div>}
          <div style={{display:"flex",flexDirection:"column",gap:7}}>
            {tareasVis.length===0&&<div style={{textAlign:"center",padding:"30px 0",color:COLORS.gray}}>No hay tareas en esta categoría</div>}
            {tareasVis.map(tarea=>{
              const vencida=tarea.status!=="Completado"&&tarea.fechaLimite&&new Date(tarea.fechaLimite)<today;
              return(<div key={tarea.id} style={{background:COLORS.surface,border:`1px solid ${vencida?COLORS.red+"55":COLORS.border}`,borderRadius:10,padding:"10px 13px"}}>
                <div style={{display:"flex",alignItems:"flex-start",gap:10}}>
                  <button onClick={()=>toggleTarea(tarea)} style={{width:22,height:22,borderRadius:"50%",border:`2px solid ${tarea.status==="Completado"?COLORS.green:tarea.status==="En progreso"?COLORS.teal:COLORS.border}`,background:tarea.status==="Completado"?COLORS.green:tarea.status==="En progreso"?COLORS.tealLight:COLORS.surface,cursor:"pointer",flexShrink:0,marginTop:1,display:"flex",alignItems:"center",justifyContent:"center"}}>
                    {tarea.status==="Completado"&&<span style={{color:"#fff",fontSize:11}}>✓</span>}
                    {tarea.status==="En progreso"&&<span style={{color:COLORS.teal,fontSize:10}}>▶</span>}
                  </button>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{display:"flex",gap:7,flexWrap:"wrap",alignItems:"center",marginBottom:3}}>
                      <span style={{fontSize:14,fontWeight:500,color:tarea.status==="Completado"?COLORS.gray:COLORS.text,textDecoration:tarea.status==="Completado"?"line-through":"none"}}>{tarea.nombre}</span>
                      {vencida&&<span style={{fontSize:11,background:COLORS.redLight,color:COLORS.red,padding:"1px 6px",borderRadius:4,fontWeight:600}}>Vencida</span>}
                    </div>
                    <div style={{display:"flex",gap:10,fontSize:12,color:COLORS.gray,flexWrap:"wrap",alignItems:"center"}}>
                  ...
                  <div style={{display:"flex",gap:6,alignItems:"center",flexShrink:0}}>
                    ...
                    {esAdmin&&<><button onClick={()=>setModalTarea(tarea)} style={{background:"none",border:"none",cursor:"pointer",fontSize:14,padding:"2px 3px"}}>✏️</button><button onClick={()=>setConfirmDel(tarea.id)} style={{background:"none",border:"none",cursor:"pointer",fontSize:14,padding:"2px 3px",color:COLORS.red}}>🗑️</button></>}
                  </div>
                </div>
              </div>);
            })}
      ...
      {tab==="Equipo"&&(
        <div>
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(200px,1fr))",gap:12}}>
            {proyecto.equipo.map((persona,i)=>(<div key={i} style={{background:COLORS.surface,border:`1px solid ${COLORS.border}`,borderRadius:12,padding:16,display:"flex",gap:12,alignItems:"center"}}><Avatar nombre={persona.nombre} size={40}/><div><div style={{fontSize:14,fontWeight:600,color:COLORS.text}}>{persona.nombre.split(" ")[0]}</div><div style={{fontSize:11,color:COLORS.gray}}>{persona.nombre.split(" ").slice(1).join(" ")}</div><div style={{marginTop:4}}><Pill label={persona.rol} color={COLORS.teal}/></div></div></div>))}
          </div>
          <div style={{marginTop:16,background:COLORS.grayLight,borderRadius:10,padding:14}}><div style={{fontSize:13,fontWeight:600,color:COLORS.text,marginBottom:6}}>Maquilador</div><div style={{fontSize:13,color:COLORS.gray}}>🏭 <b style={{color:COLORS.text}}>{proyecto.maquilador}</b> · {proyecto.pais}</div></div>
        </div>
      )}
      {tab==="Minutas"&&(
        <div>
          <div style={{display:"flex",justifyContent:"space-between",marginBottom:14}}>
            <span style={{fontSize:13,color:COLORS.gray}}>{proyecto.minutas.length} minuta(s)</span>
            <Btn onClick={()=>setModalMinuta(true)}>+ Nueva minuta / Unirse a Teams</Btn>
          </div>
          {proyecto.minutas.length===0&&<div style={{textAlign:"center",padding:"36px 0",color:COLORS.gray}}>No hay minutas registradas</div>}
          {proyecto.minutas.map(m=>(<div key={m.id} style={{background:COLORS.surface,border:`1px solid ${COLORS.border}`,borderRadius:12,padding:16,marginBottom:12}}>
            <div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><span style={{fontSize:15,fontWeight:700,color:COLORS.text}}>{m.titulo}</span><span style={{fontSize:12,color:COLORS.gray}}>{m.fecha}</span></div>
            ...
            <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:10}}>{m.asistentes.map(a=><div key={a} style={{display:"flex",alignItems:"center",gap:5,fontSize:12,color:COLORS.gray}}><Avatar nombre={a} size={18}/>{a.split(" ")[0]}</div>)}</div>
            {m.acuerdos.length>0&&<div style={{marginBottom:8}}><div style={{fontSize:12,fontWeight:600,color:COLORS.text,marginBottom:5}}>Acuerdos</div>{m.acuerdos.map((a,i)=><div key={i} style={{fontSize:13,color:COLORS.gray,paddingLeft:10,borderLeft:`2px solid ${COLORS.teal}`,marginBottom:5}}>✓ {a}</div>)}</div>}
            {m.proximos.length>0&&<div><div style={{fontSize:12,fontWeight:600,color:COLORS.text,marginBottom:5}}>Próximos pasos</div>{m.proximos.map((p,i)=><div key={i} style={{fontSize:13,color:COLORS.gray,paddingLeft:10,borderLeft:`2px solid ${COLORS.amber}`,marginBottom:5}}>→ {p}</div>)}</div>}
          </div>))}
        </div>
      )}
...
export default function App(){
  ...
  // ── Loading / Auth ──
  if(loading) return(
    <div style={{minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",background:COLORS.bg,flexDirection:"column",gap:16}}>
      <Spinner/><div style={{fontSize:14,color:COLORS.gray}}>Cargando…</div>
    </div>
  );
  ...
  if(proyectoActivo){
    ...
    return(
      <div style={{background:COLORS.bg,minHeight:"100vh"}}>
        {saving&&<div style={{position:"fixed",top:12,right:16,background:COLORS.teal,color:"#fff",borderRadius:8,padding:"5px 14px",fontSize:12,zIndex:99}}>Guardando…</div>}
    ...
    <div style={{fontFamily:"'DM Sans','Segoe UI',system-ui,sans-serif",maxWidth:880,margin:"0 auto",padding:"0 16px 40px",minHeight:"100vh"}}>
      ...
      <div style={{paddingTop:20,marginBottom:20}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10}}>
          <div>
            <div style={{fontSize:11,letterSpacing:"0.12em",textTransform:"uppercase",color:COLORS.teal,fontWeight:700,marginBottom:4}}>Gestión de proyectos</div>
            <div style={{fontSize:26,fontWeight:900,color:COLORS.text,letterSpacing:-1}}>Marycarmen Reséndiz</div>
            <div style={{fontSize:14,color:COLORS.gray,marginTop:2}}>OTC · Cosméticos · Manufactura</div>
          </div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
            {/* Usuario logueado */}
            <div style={{display:"flex",alignItems:"center",gap:8,padding:"6px 12px",background:COLORS.grayLight,borderRadius:20,fontSize:13}}>
              <Avatar nombre={perfil?.nombre||perfil?.email||"?"} size={22}/>
              <span style={{color:COLORS.text,fontWeight:500}}>{perfil?.nombre||perfil?.email}</span>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:10,marginBottom:20}}>
        {[{label:"Proyectos totales",value:stats.total,color:COLORS.text},{label:"En proceso",value:stats.enProceso,color:COLORS.teal},{label:"Tareas vencidas",value:stats.vencidos,color:stats.vencidos>0?COLORS.red:COLORS.green},{label:"Avance promedio",value:`${stats.avg}%`,color:COLORS.amber}].map(m=>(<div key={m.label} style={{background:COLORS.surface,borderRadius:12,padding:"14px 16px",border:`1px solid ${COLORS.border}`}}><div style={{fontSize:11,color:COLORS.gray,marginBottom:4,textTransform:"uppercase",letterSpacing:"0.04em"}}>{m.label}</div><div style={{fontSize:24,fontWeight:800,color:m.color}}>{m.value}</div></div>))}
      </div>

      {/* Filtros */}
      <div style={{display:"flex",gap:10,marginBottom:16,flexWrap:"wrap"}}>
        <input placeholder="Buscar producto, cliente o maquilador…" value={busqueda} onChange={e=>setBusqueda(e.target.value)} style={{flex:1,minWidth:200,padding:"9px 13px",borderRadius:9,border:`1px solid ${COLORS.border}`,fontSize:13,outline:"none",background:COLORS.surface,fontFamily:"inherit"}}/>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10}}>
        ...
        {filtrados.length===0&&<div style={{textAlign:"center",padding:"40px 0",color:COLORS.gray}}>{proyectos.length===0?"Cargando proyectos…":"No se encontraron proyectos"}</div>}
      </div>
